...
cdef complex spectral0(double w, double t): return 0 if (w > -1e-4 and w < 1e-4) else (w > 0) * 0.008944271909999158*w**3* (1 + exp(-100*w)) /           (1-exp(-100*w))+ (w < 0) * 0.008944271909999158*w**3* exp(100*w) /             (1-exp(100*w))
...
def cy_td_ode_rhs(
    ...
    dense_add_mult(H, H1, 1 + np.sign(np.sin(10*t)))
