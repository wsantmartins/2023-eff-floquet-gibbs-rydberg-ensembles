...
include 'C:/Users/Wilson/anaconda3/envs/qutip_pc/Lib/site-packages/qutip/cy/complex_math.pxi'

cdef complex spectral0(double w, double t): return 0 if (w > -1e-4 and w < 1e-4) else (w > 0) * 0.008944271909999158*w**3* (1 + exp(-10*w)) /           (1-exp(-10*w))+ (w < 0) * 0.008944271909999158*w**3* exp(10*w) /             (1-exp(10*w))
...
def cy_td_ode_rhs(
    ...
    dense_add_mult(H, H1, 0.5 + 0.5*np.sign(np.sin(8*t)))
