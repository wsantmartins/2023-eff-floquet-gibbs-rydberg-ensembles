...
cdef complex spectral0(double w, double t): return 0 if (w > -1e-4 and w < 1e-4) else (w > 0) * (4* w**3) / (3*1*1**3)* (1 + exp(-1.0*1*w)) /           (1-exp(-1.0*1*w))+ (w < 0) * (4* w**3) / (3*1*1**3)* exp(1.0*1*w) /             (1-exp(1.0*1*w))
...
def cy_td_ode_rhs(
    ...
    dense_add_mult(Ac0, A0, exp(100j*t))
    dense_add_mult(Ac0, A1, exp(-100j*t))
